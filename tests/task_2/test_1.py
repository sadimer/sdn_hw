...
def mininet():
    ...
    s1.start([c])
    s2.start([c])
    yield net, s1, s2, h1, h2, h3, h4, h5, h6
    for mnobj in net, s1, s2, h1, h2, h3, h4, h5, h6:
        mnobj.stop()
...
def test_connectivity_1(mininet, controller):
    net, s1, s2, h1, h2, h3, h4, h5, h6 = mininet
    r1 = net.ping([h1, h2])
    r2 = net.ping([h1, h6])
    r3 = net.ping([h2, h6])
...
def test_mac_table(mininet, controller):
    assert filecmp.cmp("tests/task_2/test_1.out", "task_2.out") == True
