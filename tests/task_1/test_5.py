...
def mininet():
    ...
    yield net, s1, s2, s3, h1, h2, h3, h4
    for mnobj in net, s1, s2, s3, h1, h2, h3, h4:
        mnobj.stop()
...
def test_connectivity(mininet, controller):
    net, s1, s2, s3, h1, h2, h3, h4 = mininet
    r1 = net.ping([h1, h2])
    r2 = net.ping([h1, h3])
    r3 = net.ping([h1, h4])
    r4 = net.ping([h2, h3])
...
def test_mac_table(mininet, controller):
    assert filecmp.cmp("tests/task_1/test_5.out", "task_1.out") == True
